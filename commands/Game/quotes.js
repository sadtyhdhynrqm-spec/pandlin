...
فليس ڪل مريض يآخذ نفس دوآء💔💙↗️`,
        ...
        try {
          const waifuCategories = ["waifu", "neko", "megumin", "shinobu"];
          const cat = waifuCategories[Math.floor(Math.random() * waifuCategories.length)];
          const waifuRes = await axios.get(`https://api.waifu.pics/sfw/${cat}`, { timeout: 8000 });
          const imageUrl = waifuRes.data.url;
          ...
          await api.sendMessage({ body: randomMessage }, event.threadID);
        }
